...
func (l Logger) EditIteractionResponse(sess *discordgo.Session, i *discordgo.InteractionCreate) (string, error) {
	...
	defer func(begin time.Time, output string) {
		l.Logger.Printf("command=%s edited interaction response='%s' err=%s took=%s", l.Next.ApplicationCommand().Name, output, err, time.Since(begin))
	}(time.Now(), output)
...
func (l Logger) HandlerFunc(sess *discordgo.Session, i *discordgo.InteractionCreate) (string, error) {
	...
	defer func(begin time.Time, output string) {
		l.Logger.Printf("command=%s response='%s' err=%s took=%s", l.Next.ApplicationCommand().Name, output, err, time.Since(begin))
	}(time.Now(), output)
...
func (l Logger) InteractionApplicationCommandAutocomplete(sess *discordgo.Session, i *discordgo.InteractionCreate) (string, error) {
	...
	defer func(begin time.Time, output string) {
		l.Logger.Printf("command=%s auto complete response='%s' err=%s took=%s", l.Next.ApplicationCommand().Name, output, err, time.Since(begin))
	}(time.Now(), output)
